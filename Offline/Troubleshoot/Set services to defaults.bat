...
"Start"=dword:00000003
"@
	Set-Content -Path "$env:TEMP\DefaultServices.reg" -Value $MultilineComment -Force
	# disable services RunAsTI
	$DefaultServices = @'
	Regedit.exe /S "$env:TEMP\DefaultServices.reg"
'@
	RunAsTI powershell "-nologo -windowstyle hidden -command $DefaultServices"
